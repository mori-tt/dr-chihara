...
「私の哲学」は、千原良友先生の対談を掲載するための一覧・記事テンプレートです。外部CMSやサーバー処理は使用せず、記事データから3言語の静的HTMLを生成します。
...
サンプルは実際の対談ではありません。公開日やゲストの実績・発言は架空で補っていません。カウンセリングルーム・待合室の写真は、クリニック公式サイトから取得済みの仮画像です。サンプル詳細は `noindex, follow` とし、サイトマップ・公開記事の構造化データから除外しています。URLを知っている人は閲覧でき、アクセス制限はありません。

## 記事を追加する場所

`src/lib/dialogues.ts` の `dialogues` 配列が記事の管理場所です。`sample` のオブジェクトを複製し、1記事分として配列に追加します。全言語で共通の英数字・ハイフンの `slug` を指定してください。
